...
# This script is designed to create deterministic forecasts using the NeuralForecaster library
...
if __name__ == "__main__":
    ...
    for building in BUILDINGS:
        ...
        models = [
        AutoKAN(h=HORIZON, loss=MSE(), num_samples=NUM_SAMPLES, config=config_kan, valid_loss=MSE(), search_alg=search_algorithm)
        ]
        ...
        for model in nf.models:
            ...
            forecast_names = [f'P_TOT_HAT']
            columns[len(base_cols):len(base_cols) + len(forecast_names)] = forecast_names
            model_df.columns = columns

            output_file = f"{output_dir}/file_fc_{model_name}Det_MSE_{building}_{TIME_STAMP}_freq{RESOLUTION}.csv"
