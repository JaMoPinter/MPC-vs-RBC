...
if __name__ == "__main__":
    ...
    for building in BUILDINGS:
        ...
        MAX_EPOCHS = 800  # Set a fixed number of epochs for all models
        ...
        config_patch_tst["max_steps"] = tune.choice([MAX_EPOCHS])
        ...
        config_tsmixerx.update({
            ...
            "max_steps": tune.choice([MAX_EPOCHS]),
            "windows_batch_size": tune.choice([64, 128]),
            "batch_size": tune.choice([BATCH_SIZE]),  # vermutlich 1
            "val_check_steps": tune.choice([10]),
            "early_stop_patience_steps": tune.choice([5]),
            "random_seed": random_seed,
        ...
        config_tide.update({
            "input_size": tune.choice([HORIZON * 3, HORIZON * 7]),
            "max_steps": tune.choice([MAX_EPOCHS]),
            "windows_batch_size": tune.choice([64, 128]),      # kleiner als default!
            "batch_size": tune.choice([1]),
            "val_check_steps": tune.choice([10]),
            "early_stop_patience_steps": tune.choice([5]),
            "dropout": tune.choice([0.1, 0.2]),                
            "hidden_size": tune.choice([64, 128]),             # statt 256+ (falls enthalten)
            "learning_rate": tune.choice([1e-3, 5e-4]),
            "random_seed": random_seed,
        })

        # wichtig: input_size_multiplier raus
        ...
        config_kan.update({
            "input_size": tune.choice([HORIZON * 3, HORIZON * 5]),
        ...
        for model_class, best_config in best_configs:  # best_configs = [(PatchTST, config_patch), (NHITS, config_nhits), ...]
            ...
            # delete early stopping parameters
            best_config.pop("early_stop_patience_steps", None)
            best_config.pop("val_check_steps", None)
