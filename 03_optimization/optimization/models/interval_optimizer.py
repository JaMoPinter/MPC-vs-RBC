...
class IntervalOptimizer(BaseOptimizer):
    ...
    def __init__(self, battery_cfg: dict, mpc_freq: int, prices, param_assumption: str = None):
        super().__init__(battery_cfg=battery_cfg, mpc_freq=mpc_freq, prices=prices, param_assumption=param_assumption) # only necessary if this class has its own init method
    ...
    def update_soe(self, t_now, decision, gt):
        ...
        soe_new = self.soe_now - pb_now * self.t_inc * eta
